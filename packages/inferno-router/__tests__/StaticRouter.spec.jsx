...
import { StaticRouter, Redirect, Route } from "inferno-router";
...
describe('A <StaticRouter>', () => {
  ...
  it.skip('warns when passed a history prop', () => {
    const context = {}
    const history = {}
    ...
    //expect(console.warn.calls.mostRecent().args[0]).toContain('<StaticRouter> ignores the history prop')
  ...
  it('knows how to parse raw URLs', () => {
    const LocationChecker = (props) => {
      expect(props.location.pathname).toBe('/the/path')
      expect(props.location.search).toBe('?the=query')
      expect(props.location.hash).toBe('#the-hash')
